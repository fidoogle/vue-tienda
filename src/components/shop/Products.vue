...
<script>
  ...
  import ProductSheet from './ProductSheet';
    ...
    data () {
		  return {
		    loading: false, //for loading bar that shows
        productSheet: {},
        statusSheet: false
      }
    },
    destroyed () {
		  this.$store.commit('resetProductsPagination'); //defined in modules/shop
    },
    computed: {
		  products () {
    ...
    methods: {
      ...
      fetchProducts () {
		    if ( !this.$store.state.shopModule.finish) {
		      this.loading = true;
		      setTimeout(() => {
		        this.$store.dispatch('fetchAllProductsPaginate').then(() => { //defined in modules/shop
		          this.loading = false;
            })
          }, 2000);
        }
      ...
      changeLimit (limit) {
		    if ( this.limit !== limit) {
		      ...
		      this.fetchProducts(); //notice call to reload
        }
      },
      closeSheet () {
